...
| Network security headers | **Present on API responses and, as of 2026-09, on pages** — every Worker sends `content-security-policy`, `x-frame-options`, `x-content-type-options`, `referrer-policy`, and `permissions-policy` (see `docs/SUBPROCESSORS.md` for the CSP host allowlist and the rationale for `widget.js`'s intentionally permissive CORS on its one public, non-sensitive endpoint). **Correction to the previous entry**: those headers were declared in Worker code and reached API responses only. Every wrangler config sets `run_worker_first: ["/api/*"]`, so Cloudflare's asset server answered page requests directly and the Worker never ran — every HTML page on every surface was served with none of these headers, for as long as this row has claimed otherwise. Found by curling production, not by reading the code, which is the lesson. `scripts/sync-headers.mjs` now generates a `_headers` file per assets directory from each Worker's own exported constants, and `npm run check` fails when a file and its Worker disagree. |
