...
| **`GEMINI_API_KEY` on the voice Worker** (status unknown — the voice health endpoint now reports it). | Without it every clinic call speaks Twilio's built-in voice. The default is now Polly's generative tier (a big step up from the old neural Joanna), but the truly natural, directable voice is the Gemini TTS path, and it is a single key. | `npx wrangler secret put GEMINI_API_KEY --config wrangler.voice.jsonc` with an AI Studio key. After deploying, `curl https://voice.timinow.pet/api/health` — `speechPath` should read `gemini-tts`. |
