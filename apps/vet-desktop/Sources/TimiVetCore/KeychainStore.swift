...
/// Stores the Clerk bearer token in the macOS Keychain instead of the settings
/// JSON file. The Windows app writes its bearer token straight into
/// settings.json — its own README calls that a dev-only shortcut — so this
/// port deliberately does not repeat it.
///
/// Guarded with `#if canImport(Security)` and backed by an in-memory
/// dictionary otherwise, so TimiVetCore still compiles with the Apple-only
/// Keychain APIs stripped (e.g. under Skip Fuse on a non-Apple platform).
public final class KeychainStore: @unchecked Sendable {
    private let service = "solutions.clearkey.timivet"
