...
@MainActor @Observable public final class AppStore {
    ...
    /// they cross through: Stripe's own error taxonomy (type/code — never
    /// card data, never a client secret), reported through the same
    /// cookieless analytics pipeline as everything else, so a payment
